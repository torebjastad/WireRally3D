...
def test_ai_driver_completes_rally_course():
    ...
    Verify that AI driver triggers all checkpoints within realistic time limit (< 95 seconds).
    """
    success, lap_time, trajectory = simulate_rally_lap(max_time=95.0)
    ...
    assert success, "AI driver failed to navigate the full rally course within 95 seconds"
    assert 25.0 < lap_time < 95.0, f"Lap time outside reasonable range: {lap_time:.2f}s"
